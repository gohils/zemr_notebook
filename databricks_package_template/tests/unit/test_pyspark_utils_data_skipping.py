...
def test_data_skipping1(spark_session: SparkSession):
    ...
    assert spark_session.conf.get("spark.databricks.delta.properties.defaults.dataSkippingNumIndexedCols") == "2"
...
def test_data_skipping2(spark_session: SparkSession):
    ...
    assert spark_session.conf.get("spark.databricks.delta.properties.defaults.dataSkippingNumIndexedCols") == "3"
...
def test_data_skipping3(spark_session: SparkSession):
    ...
    assert processed_df.columns == ["col2", "col4", "col1", "col3"]
    assert spark_session.conf.get("spark.databricks.delta.properties.defaults.dataSkippingNumIndexedCols") == "1"
