...
async def create_customer_record(
    customer_data: CustomerOnboardingRequest
) -> CustomerRecord:
...
@activity.defn
async def update_customer_status(customer_id: str, status: str) -> str:
    """Updates customer status in database."""
    print("\n[DB] update_customer_status INPUT:",
...
async def run_kyc_check(customer: CustomerRecord) -> KYCResult:
    """Calls external KYC verification API."""
...
async def run_credit_check(customer: CustomerRecord) -> CreditResult:
    """Calls external credit bureau API."""
...
async def provision_bank_account(customer: CustomerRecord) -> str:
    """Calls core banking API to provision account."""
...
class CustomerOnboardingWorkflow:
    """Orchestrates full customer onboarding lifecycle."""

    @workflow.run
    async def run(
        self,
        customer_input: CustomerOnboardingRequest
    ) -> OnboardingResponse:
        """Executes onboarding workflow with sequential, parallel, conditional, loop and timeout logic."""

        print("\n=== WORKFLOW STARTED WITH INPUT ===")
        print(customer_input.model_dump())

        # ==================================================
        # 1️⃣ Sequential Step — Create Customer
        # ==================================================
        ...
        # ==================================================
        # 2️⃣ Parallel Execution — KYC + Credit Check
        # ==================================================
        ...
        kyc_result, credit_result = await asyncio.gather(
            kyc_future,
            credit_future
        )
        ...
        # ==================================================
        # 4️⃣ Loop — Simulate Polling Until KYC Approved
        # ==================================================
